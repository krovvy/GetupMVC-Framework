...
function get_debug() {
  ...
  if(isset($ly->config['debug']['lydia']) && $ly->config['debug']['lydia']) {
    $html .= "<hr><h3>Debuginformation</h3><p>The content of CGetupMVC:</p><pre>" . htmlent(print_r($ly, true)) . "</pre>";
  }
  if(isset($ly->config['debug']['session']) && $ly->config['debug']['session']) {
    $html .= "<hr><h3>SESSION</h3><p>The content of CGetupMVC->session:</p><pre>" . htmlent(print_r($ly->session, true)) . "</pre>";
...
function get_messages_from_session() {
  $messages = CGetupMVC::Instance()->session->GetMessages();
  $html = null;
  if(!empty($messages)) {
...
function get_gravatar($size=null) {
  return 'http://www.gravatar.com/avatar/' . md5(strtolower(trim(CGetupMVC::Instance()->user['email']))) . '.jpg?r=pg&amp;d=wavatar&amp;' . ($size ? "s=$size" : null);
}
...
function base_url($url=null) {
  return CGetupMVC::Instance()->request->base_url . trim($url, '/');
}
...
function create_url($urlOrController=null, $method=null, $arguments=null) {
  return CGetupMVC::Instance()->CreateUrl($urlOrController, $method, $arguments);
}
...
function theme_url($url) {
  return create_url(CGetupMVC::Instance()->themeUrl . "/{$url}");
}
...
function theme_parent_url($url) {
  return create_url(CGetupMVC::Instance()->themeParentUrl . "/{$url}");
}
...
function current_url() {
  return CGetupMVC::Instance()->request->current_url;
}
...
function render_views($region='default') {
  return CGetupMVC::Instance()->views->Render($region);
}
...
function region_has_content($region='default' /*...*/) {
  return CGetupMVC::Instance()->views->RegionHasView(func_get_args());
}
